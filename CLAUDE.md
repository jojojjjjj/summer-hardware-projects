...
> The website has an upgrade series in `prompts/`: **05** (visual — warm→cool Meta palette, glass, glow) → **06** (interactive — V2: global mouse, scroll divider, parallax) → **07** (architecture & flow — **active**: lifts 05/06's no-restructure constraint; View Transitions page-flow + shared-element morph, 3D signature hero, de-templated section rhythm, detail-page rework) → **08** (media & effects — **active**: selectively integrates user-supplied art/video assets from `D:\教书\艺术设计图片` + cherry-picked effects from `prompts/网页效果提示词/` — mouse-scrub video, video+noise bg, WordsPullUp, scroll-linked char reveal; layers on top of 07, not a global rebuild) → **09** (hero fusion — **active**: fuses a mouse-following mechanical figure into the existing 3D constellation hero **in the same Three.js scene** (`HeroArtifact.vue`), harmony-first, with an automated multi-agent jury-score loop that must clear >8.8/10 before reporting) → **10** (hero figure → GLTF — **active, supersedes 09's primitive figure**: 09 built the figure from ≤12 code primitives, which looked toy-like vs the cinematic `hero page 小人.txt` reference; 10 replaces it with a real **high-fidelity GLTF/GLB model** loaded into the same Three.js scene, head tracks mouse via real 3D pivot/bone rotation; model sourced via AI-generate (Meshy/Tripo) or free libs (Sketchfab/Quaternius); harmony constraints + jury >8.8 preserved). When touching visual design/architecture/media/hero, read `prompts/07_website_architecture_upgrade.md`, `prompts/08_website_media_effects_upgrade.md`, `prompts/09_hero_mechanical_figure_fusion.md` (lineage), and `prompts/10_hero_figure_gltf_upgrade.md` (current figure direction) first; tokens/glass/glow from 05/06 are done — don't rebuild them.
>
...
The pipeline was used to produce the initial projects; the prompts in `prompts/` are the source of truth for re-running or modifying the workflow. `prompts/05_website_visual_upgrade.md`, `06_website_interactive_upgrade.md`, `07_website_architecture_upgrade.md`, `08_website_media_effects_upgrade.md`, `09_hero_mechanical_figure_fusion.md`, and `10_hero_figure_gltf_upgrade.md` form a separate, website-only upgrade series (not part of the course-pack pipeline); **07/08/09/10 are the active rounds** (07 = architecture/3D/flow, 08 = media/effects, 09/10 = hero figure fusion → GLTF).
...
- **Visual verification & design-audit artifacts**: visual-verification screenshots, capture scripts, and design-review reports live under **`verify/`** (gitignored) — see [Visual verification](#visual-verification). `website-research/` contains competitive analysis and design specs — reference it (and the `prompts/05→06→07→08→09→10` upgrade series, **07/08/09/10 active**, 09/10 = current hero direction) when modifying visual design, site architecture, or media/effects. User-supplied art/video assets live in `D:\教书\艺术设计图片` (outside the repo) and are ingested into `website/public/media/` per `prompts/08`. (The old `website-audit/` folder is gone; do not recreate it.)
