...
The pipeline was used to produce the initial projects; the prompts in `prompts/` are the source of truth for re-running or modifying the workflow. `prompts/05_website_visual_upgrade.md`, `06_website_interactive_upgrade.md`, `07_website_architecture_upgrade.md`, `08_website_media_effects_upgrade.md`, `09_hero_mechanical_figure_fusion.md`, `10_hero_2dvideo_3dconstellation.md`, and `12_course_stages_upgrade.md` form a separate, website-only upgrade series (not part of the course-pack pipeline); **07/08/10/12 are the active rounds** (07 = architecture/3D/flow, 08 = media/effects, 10 = hero 2D-scrub-video + 3D constellation, 12 = course-stages section [scoped — ONLY `JackServices.vue`, everything before it FROZEN]). 09 is lineage (hero figure, superseded).
