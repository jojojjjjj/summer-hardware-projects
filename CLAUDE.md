...
The pipeline was used to produce the initial projects; the prompts in `prompts/` are the source of truth for re-running or modifying the workflow. `prompts/05_website_visual_upgrade.md`, `06_website_interactive_upgrade.md`, `07_website_architecture_upgrade.md`, `08_website_media_effects_upgrade.md`, `09_hero_mechanical_figure_fusion.md`, and `10_hero_2dvideo_3dconstellation.md` form a separate, website-only upgrade series (not part of the course-pack pipeline); **07/08/10 are the active rounds** (07 = architecture/3D/flow, 08 = media/effects, 10 = hero 2D-scrub-video + premium un-clipped 3D constellation, figure dropped). 09 is lineage (hero figure, superseded).
