...
The pipeline was used to produce the initial projects; the prompts in `prompts/` are the source of truth for re-running or modifying the workflow. `prompts/05_website_visual_upgrade.md`, `06_website_interactive_upgrade.md`, `07_website_architecture_upgrade.md`, `08_website_media_effects_upgrade.md`, `09_hero_mechanical_figure_fusion.md`, `10_hero_2dvideo_3dconstellation.md`, `12_course_stages_upgrade.md`, `13_mission_blackhole_video_upgrade.md`, `14_mobile_optimization.md`, `15_project_links_upstream_primary.md`, `16_image_loading_optimization.md`, `17_project02_redesign.md`, `18_safari_adaptation.md`, and `19_course_review_panel.md` form a separate, website-only upgrade series (not part of the course-pack pipeline); **07/08/10/12/13/14/15/16/17/18/19 are the active rounds** (07 = architecture/3D/flow, 08 = media/effects, 10 = hero 2D-scrub-video + 3D constellation, 12 = course-stages [scoped], 13 = mission section [scoped], 14 = mobile-only [≥768 FROZEN], 15 = project links [upstream-primary], 16 = image loading [signup-focused], 17 = project-02 redesign, 18 = Safari adaptation, 19 = course-design review panel [experiential bar — rough hands-on breadth not deep mastery; references `example_course_course_work/` structure but tunes depth down; plain-language no bureaucratic tone; agent freely designs ultracode/Workflow dynamic workflow; fixes over-deep/jargon/no-hands-on courses via submodule workflow; produces `REVIEW_REPORT_V2`]). 09 is lineage (hero figure, superseded).
